...
if(element.variants)
{
    ...
    this.service.updateProduct(model).subscribe(res => {
     //alert(JSON.stringify(res))
      ...
      this.service.checkWhishlistOutFStock(model).subscribe(x=>{
...
var path=this.imageList[i].images[j].image;
  ...
  onUploadSuccess(event: any) {
    const typeFile = event[0].dataURL.split('/')
    const type=typeFile[1].split(';')
    if(type[0] === 'png' ){
      this.toastr.info("Do not support .png image")
      return false;
    }
    if (event[0].dataURL) {
